                ...
                for d in j['body_text']:
                    if d['section']=='Keywords':
                        ...
                        for dic in j['metadata']['authors']:
                            ...
                            name += ', '
                        name = name[:-2]
                        corona_info.append([j['paper_id'], j['metadata']['title'], name, i, 0])
...
def generate_histogram(corona_pos_all_text):
    ...
    plt.hist(corona_hist,bins=20,color='#0504aa',alpha=0.7)
...
def generate_csv(corona_info):
    corona_shuffled = random.sample(corona_info, len(corona_info))[:300]

    file=open('labels.csv','w+',newline='',encoding='utf-8')
    # identifying header
    header=[['paper_number','paper_id','title','authors','keyword_match']]

    # writing data row-wise into the csv file
    with file:
        write=csv.writer(file)
        write.writerows(header)
        write.writerows(corona_shuffled)
...
corona_pos_all_text, corona_info, keywords_cnt, keywords_list = preprocess_data()

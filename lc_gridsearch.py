...
def tfidf_LDA(corona_body_all_text):
    ...
    # the features of each data are 1500 now
    # this is the TF-IDF for Bigrams and Trigrams
...
def add_noise(corpus_matrix):
    ...
    for index in range(40):
        ...
        ini_training_y, ini_training_indices, _ = generate_development_set('2500_labels.xlsx')
        ini_training_y = np.array(ini_training_y)

        cw = collections.Counter(ini_training_y)

        parameters = [
            {'kernel': ['rbf'], 'gamma': [1, 0.1, 1e-2, 1e-3, 1e-4], 'C': [0.001, 0.01, 0.1, 1, 10, 100, 1000],
             'class_weight': [cw]},
            {'kernel': ['linear'], 'C': [0.001, 0.01, 0.1, 1, 10, 100, 1000], 'class_weight': [cw]}]

        clf = GridSearchCV(SVC(), parameters, refit=True, verbose=3, scoring='f1', n_jobs=6)
        ...
        ini_test_y, ini_test_indices = generate_labels('500_samples.xlsx')
    ...
    for k in range(1,11):
        ...
        develop_y, develop_indices, pos=generate_development_set('2500_labels.xlsx')
        """
        ...
        """
        ref = list(set([i for i in range(1,2501)]) - set(pos))
        ...
        for pos in range(40):
            ...
            random_y, random_indices = extract_info('2500_labels.xlsx',random_pos)
            ...
            parameters = [
                {'kernel': ['rbf'], 'gamma': [1, 0.1, 1e-2, 1e-3, 1e-4], 'C': [0.001, 0.01, 0.1, 1, 10, 100, 1000],
                 'class_weight': [dic]},
                {'kernel': ['linear'], 'C': [0.001, 0.01, 0.1, 1, 10, 100, 1000], 'class_weight': [dic]}]
            ...
            test_y, test_indices = generate_labels('500_samples.xlsx')
            test_X = []
...
def plot_confidence_intervals(training_error,test_error,image_id,y_name):
    ...
    # 280 is equal to (3000-200)//10, 3000 data in training dataset, 200 data in development set and 10 models
    X=[200+i*230 for i in range(11)]
...
np.save('trigrams_wordfeature_list.npy',word_feature_list,allow_pickle=True)
np.save('trigrams_corpus_matrix.npy',corpus_matrix,allow_pickle=True)
